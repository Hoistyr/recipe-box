...
const EditRecipeForm = (props) => {
  ...
  const [missingInformationError, setMissingInformationError] = useState('');
  ...
  useEffect(() => {
    ...
    setToolList(recipe.tools);
    setTagList(recipe.tags);
    setDirectionsList(recipe.directions);
  }, [recipe]);
  ...
  const saveRecipe = (event) => {
    ...
    const prepTime = {
      hour: Number(form.querySelector('.prepTimeHour').value) || 0,
      minute: Number(form.querySelector('.prepTimeMinute').value) || 0,
    };
    const cookTime = {
      hour: Number(form.querySelector('.cookTimeHour').value) || 0,
      minute: Number(form.querySelector('.cookTimeMinute').value) || 0,
    };
    const servings = Number(form.querySelector('.servings').value) || 0;
    const cookingDevice = form.querySelector('.cookingDevice').value || '';
    const ovenTemp = Number(form.querySelector('.ovenTemp').value) || 0;
    ...
    const recipe = {
      info: {
        name: recipeName || '',
        lowName: recipeName.toLowerCase() || '',
      ...
      id: editRecipe.id,
      dateCreated: editRecipe.dateCreated,
      dateUpdated: editRecipe.dateUpdated,
      nextDatesPlanned: editRecipe.nextDatesPlanned,
      prevDatesPlanned: editRecipe.prevDatesPlanned,
      creatorName: editRecipe.creatorName, 
    }

    setRecipeInformation({...recipe});
    console.log('presubmit recipe', recipe);
    // Checks if necessary information is in the recipe and then submits it to the database
    if (recipe.info.name !== '' && recipe.ingredients.length > 0 && recipe.directions.length > 0) {
      try {
        dispatch(updateRecipe(recipe.id, recipe))
      ...
      // Adds error text to tell the user what information is needed before they can submit the recipe
      let missingThings = [];
      if (recipe.info.name === '') {
        missingThings.push('a name');
      }
      if (recipe.ingredients.length < 1) {
       missingThings.push('at least one ingredient');
      }
      if (recipe.directions.length < 1) {
        missingThings.push('at least one direction');
      }

      console.log('missingThings: ', missingThings);
      console.log('missingThings: ', missingThings.length);
      let errorText = ''
      missingThings.forEach((text, index) => {
        if (missingThings.length === 1) {
          errorText = text;
        }
        
        if (missingThings.length === 2) {
          if (index === missingThings.length - 1) {
            errorText = errorText + ' and ' + text;
            return;
          }
          errorText = errorText + ' ' + text;
        }

        if (missingThings.length === 3) {
          if (index === missingThings.length - 1) {
            errorText = errorText + ' and ' + text;
            return;
          }

          if (index === missingThings.length - 2) {
            errorText = errorText + ' ' + text;
      ...
      setMissingInformationError(`Recipe must have ${errorText}`);
    }
  }
  ...
  if (submitted === true) {
    return <Redirect to={{
      pathname: `/recipes/view/${recipe.id}`
    }} />
  }
  ...
  return (
    <div className="newRecipe">
      <form className="newRecipeForm">
        ...
        <p className="submitErrorText">{missingInformationError}</p>
        <div className="recipeMainInformation formSection">
          ...
          <div className="prepTimeInputDiv">
            <input 
              ...
              name="prepTime"
              placeholder="0"
              defaultValue={editRecipe.info.prepTime.hour}
            />
            ...
            <input 
              ...
              name="prepTime"
              placeholder="0"
              defaultValue={editRecipe.info.prepTime.minute}
            />
          ...
          <div className="cookTimeInputDiv">
            <input 
              ...
              name="cookTime"
              placeholder="0"
              defaultValue={editRecipe.info.cookTime.hour}
            />
            <p className="timeText">hour(s)</p>
            <input 
              ...
              name="cookTime"
              placeholder="0"
              defaultValue={editRecipe.info.cookTime.minute}
            />
            <p className="timeText">minute(s)</p>
          </div>
          <p>Servings:</p>
            <input 
              ...
              name="servings"
              placeholder="0"
              defaultValue={editRecipe.info.servings}
            />
          ...
          <input 
            ...
            defaultValue={editRecipe.info.cookingDevice}
          />
           ...
           <input 
            ...
            name="ovenTemp"
            placeholder="0°"
            defaultValue={editRecipe.info.cookTemp}
          />
        </div>
        ...
        <div className="recipeSecondaryInformation formSection">
          ...
          <input 
            ...
            defaultValue={editRecipe.otherInformation.attribution}
          />
          <p>Notes:</p>
          <textarea 
          className="recipeNotes"
          placeholder="Notes about the recipe"
          defaultValue={editRecipe.otherInformation.notes}
          >
          </textarea>
          <p>Difficulty:</p>
          <select 
            ...
            defaultValue={editRecipe.otherInformation.difficulty}
          >
